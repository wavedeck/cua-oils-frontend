...
export class MediaRepository implements IMediaRepository {
    ...
    private async _fetchManyMedia(ids: number[]): Promise<MediaResponse[]> {
        let queryString = this._getParamsString({include: ids.join(",")});
        let requestUrl = `${apiConfig.baseUrl}/wp/v2/media?${queryString}`;
    ...
    private async _fetchMedia(params: MediaQueryParams): Promise<MediaResponse> {
        let requestUrl = `${apiConfig.baseUrl}/wp/v2/media/${params.id}`;
    ...
    private _getParamsString(params?: object) {
        return new URLSearchParams({...this._paginationParams, ...params}).toString();
    }
}
