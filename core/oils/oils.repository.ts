...
export class OilsRepository implements IOilsRepository {
   ...
   * @returns A promise resolving to an array of MediaResponse.
   */
  private async _fetchManyOils(
    options: FetchManyOptions,
  ): Promise<OilResponse[]> {
    const queryParams = this._constructQueryParams();
    const response = await $fetch.raw<OilResponse[]>(
      this._baseUrl + "?" + queryParams,
    );

    if (!response.ok || !response._data) {
      throw new Error(
        `Failed to fetch oils with statuscode ${response.status}: ${response._data}`,
      );
    }
    ...
    if (options.allPages && response.headers.get("X-WP-TotalPages")) {
      ...
      if (totalPages > 1) {
        ...
        for (let page = 2; page <= totalPages; page++) {
          requests.push(
            $fetch<OilResponse[]>(
              `${this._baseUrl}?${this._constructQueryParams({ page })}`,
            ),
          );
        }
